...
L'esercizio richiede di implementare un ciclo \emph{read-fork-exec-wait}:
leggere un comando da \texttt{stdin}, fare \texttt{fork()}, eseguire il
programma nel figlio con \texttt{execvp()} e aspettarne la terminazione nel
...
Il built-in \texttt{exit} viene gestito \textbf{prima} di \texttt{fork()}: se
venisse controllato nel figlio, sarebbe il figlio a uscire e non la shell.

\subsection*{Figlio: \texttt{execvp} e padre: \texttt{waitpid}}

\texttt{execvp(argv[0], argv)} sostituisce l'immagine del processo con il
...
if (pid == 0) {
     ...
     * exit(127): convenzione POSIX per "command not found". */
...
if (argv_right != NULL) {
    ...
    pid_t child1 = fork();
    if (child1 == 0) {
        /* stdout -> pipe: l'output va nel buffer invece che a schermo */
    ...
    pid_t child2 = fork();
    if (child2 == 0) {
        /* stdin <- pipe: il comando legge l'output di child1 */
